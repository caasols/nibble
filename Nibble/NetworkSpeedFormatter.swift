...
enum NetworkSpeedFormatter {
    static func string(bytesPerSecond: Double) -> String {
        ...
        let kilobytesPerSecond = bytesPerSecond / 1_024
        if kilobytesPerSecond < 1_024 {
            return String(format: "%.1f KB/s", kilobytesPerSecond)
        }

        let megabytesPerSecond = kilobytesPerSecond / 1_024
        if megabytesPerSecond < 1_024 {
            return String(format: "%.1f MB/s", megabytesPerSecond)
        }

        let gigabytesPerSecond = megabytesPerSecond / 1_024
